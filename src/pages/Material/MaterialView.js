...
const MaterialView = () => {
  ...
  const searchTable = async () => {
    const url = `${api_url}/materialBatch/name/${search}`;
    try {
  ...
  const materialsTableData = async () => {
    ...
    try {
      ...
      if (response?.status === 200) {
        setData(response?.data);
        console.log('data got Sucessfully:', response?.data);
      } else {
              ...
              >
                <div className='flex flex-row mb-4 mt-2'>
                  ...
                  <p style={{ fontWeight: '500', color: 'black' }}>
                    Are you sure you want to delete the selected Batch(s)?
                  </p>
                </div>
                <button
                  onClick={() => {
